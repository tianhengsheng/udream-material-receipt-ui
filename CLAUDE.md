...
- 一律经网关（envs.ts：默认 dev；local=本地网关，端口可在顶栏环境弹层里改，vite 插件按请求转发）；老 franchise App 接口 `/mgt/pur/**`、craftsman `/apiCraftsman/**`（网关根路由，无 /craftsman 前缀）、新接口 `/franchise/apiCraftsman|apiUnified/**` 均根路由。
