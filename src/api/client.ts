    ...
    if (body && typeof body === 'object') {
      ...
      if (c !== undefined && !ok) {
        ...
        if (TOKEN_EXPIRY_CODES.has(String(c))) {
          ...
          showTokenExpired(res.config?.url, undefined, body.retInfo || body.retMsg || body.msg);
          return Promise.reject(body);
        }
        // eslint-disable-next-line no-console
        console.warn('[resp.biz-fail]', res.config.url, body);
        if (!res.config._silent) message.error(`[${c}] ${body.retInfo || body.retMsg || body.msg || '业务异常'}`);
        return Promise.reject(body);
      }
    }
    return res;
